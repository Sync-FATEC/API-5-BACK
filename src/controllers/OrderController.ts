...
import { OrderDTO, OrderViewModel } from '../types/OrderSectionDTO';
...
export class OrderController {
		async getAll(req: Request, res: Response) {
			...
			const orders: OrderViewModel[] = await orderService.getAll(stockId);
			return res.json(orders);
		}

		async getById(req: Request, res: Response) {
			const { id } = req.params;
			const order: OrderViewModel | null = await orderService.getById(id);

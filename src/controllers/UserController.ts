...
export class UserController {
      ...
      await Promise.all(
        userTypes.map(async (userType) => {
          ...
          if (user.stockId != null) {
            await userServices.linkUserToStock(user.id, user.stockId, StockResponsibility.USER);
          }
        })
      );
  ...
  async login(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const loginResult = await userServices.loginWithEmailPassword(
        email,
        password
      );
      res.status(200).json({
  ...
  async updateUser(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const userData = await userServices.updateUser(userId, {
        name,
        email,
        role,
        isActive,
    ...
    try {
      ...
      if (!userId || !stockId || !responsibility) {
        throw new SystemError(
          "Dados incompletos para vincular usuário ao estoque"
        );
      }

      const userStock = await userServices.linkUserToStock(
        userId,
        stockId,
        responsibility
      );
  ...
  async changePassword(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      if (!email || !currentPassword || !newPassword) {
        throw new SystemError(
          "Email, senha atual e nova senha são obrigatórios"
        );
      }
